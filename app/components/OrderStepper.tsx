...
export function OrderStepper({status}: {status: FulfillmentStatusType}) {
  ...
  if (statusStr.includes('PROGRESS') || statusStr.includes('PARTIAL'))
    activeStep = 1;
  if (
    statusStr.includes('FULFILLED') &&
    !statusStr.includes('UNFULFILLED') &&
            ...
            <div key={step.title} className="flex relative items-start group">
              ...
              <div
                className={clsx(
                  ...
                  isCompleted &&
                    'bg-[#1E1E1E] dark:bg-[#F0EAE6] text-white dark:text-[#1E1E1E]',
                  isActive &&
                    'border-2 border-[#1E1E1E] dark:border-[#F0EAE6] text-[#1E1E1E] dark:text-[#F0EAE6]',
                  isPending &&
                    'border-2 border-[#e2e8f0] dark:border-white/20 text-[#94a3b8] dark:text-white/30',
                )}
              >
                {isCompleted ? (
                  <svg
