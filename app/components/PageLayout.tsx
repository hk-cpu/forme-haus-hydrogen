  ...
  return (
    <>
            ...
            >
              ...
              <div key={location.pathname} className="page-fade-in">
                  {isHome ? (
                    ...
                    <div className={`bg-[#F9F5F0]/85 backdrop-blur-md text-[#4A3C31] shadow-[0_0_80px_rgba(255,255,255,0.15)] min-h-[50vh] ${isCollectionPage ? '' : 'lg:rounded-t-[2rem]'}`}>
                      {children}
                    </div>
                  )}
              </div>
      ...
      <a
        ...
        className={`whatsapp-fab fixed bottom-[80px] md:bottom-6 z-[400] w-11 h-11 md:w-12 md:h-12 rounded-full bg-[#25D366]/70 hover:bg-[#25D366] shadow-md shadow-[#25D366]/20 hover:shadow-lg hover:shadow-[#25D366]/30 flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 ${isRTL ? 'left-4 md:left-6' : 'right-4 md:right-6'}`}
      >
...
function CartDrawer({isOpen, onClose}: {isOpen: boolean; onClose: () => void}) {
  ...
  return (
    <Drawer open={isOpen} onClose={onClose} heading="Bag" openFrom={isRTL ? 'left' : 'right'}>
      <div className="grid">
