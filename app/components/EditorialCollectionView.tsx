...
export function generateDynamicEditorialConfig(
  ...
  while (currentIndex < productCount) {
    ...
    // Use editorial-grid for batches of 10
    if (remaining >= 10) {
      sections.push({
        type: 'editorial-grid-large',
        productIndices: Array.from({length: 10}, (_, i) => currentIndex + i),
      });
      currentIndex += 10;
    }
    ...
    // Use asymmetric for batches of 4-7
    else if (remaining >= 4) {
...
function getGridClass(type: string): string {
  switch (type) {
    ...
    case 'wide':
      return 'mb-6 md:mb-8';
    case 'editorial-grid':
    case 'editorial-grid-large':
      return 'grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8';
    default:
      return '';
  }
}
