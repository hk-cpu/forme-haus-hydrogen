...
interface GhostCursorEnhancedProps {
  className?: string;
  ...
  primaryColor?: string;    // Pitch Black: #000000
  secondaryColor?: string;  // Deep Bronze: #8B5E3C or Silk: #AD9686
  hoverIntensity?: number;
}

const GhostCursorEnhanced: React.FC<GhostCursorEnhancedProps> = ({
  className,
  style,
  trailLength = 50,
  inertia = 0.5,
  ...
  const primaryThreeColor = useMemo(() => new THREE.Color(primaryColor), [primaryColor]);
  const secondaryThreeColor = useMemo(() => new THREE.Color(secondaryColor), [secondaryColor]);
  ...
  const fragmentShader = `
    ...
    uniform float iHoverIntensity;
    uniform float iIsHovering;
    ...
    vec3 blendColors(vec3 primary, vec3 secondary, float t) {
      // Dynamic blend between pitch black and bronze
      return mix(primary, secondary, t * 0.6 + 0.2 * sin(iTime * 0.5));
    }
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      // Blend between primary (black) and secondary (bronze) colors
      float colorMix = iIsHovering > 0.5 ? iHoverIntensity * 0.7 : 0.3;
      vec3 color = blendColors(iPrimaryColor, iSecondaryColor, colorMix);
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iOpacity: { value: 1.0 },
        iScale: { value: 1.0 },
        iPrimaryColor: { value: new THREE.Vector3(primaryThreeColor.r, primaryThreeColor.g, primaryThreeColor.b) },
        iSecondaryColor: { value: new THREE.Vector3(secondaryThreeColor.r, secondaryThreeColor.g, secondaryThreeColor.b) },
        iHoverIntensity: { value: hoverIntensity },
        iIsHovering: { value: 0.0 },
    ...
    const animate = () => {
      ...
      mat.uniforms.iIsHovering.value = isHoveringRef.current ? 1.0 : 0.0;
    ...
    parent.addEventListener('mouseover', onMouseOver, { passive: true });
    parent.addEventListener('mouseout', onMouseOut, { passive: true });
  ...
  useEffect(() => {
    if (materialRef.current) {
      (materialRef.current.uniforms.iPrimaryColor.value as THREE.Vector3).set(
        primaryThreeColor.r, primaryThreeColor.g, primaryThreeColor.b
      );
    }
  ...
  useEffect(() => {
    if (materialRef.current) {
      (materialRef.current.uniforms.iSecondaryColor.value as THREE.Vector3).set(
        secondaryThreeColor.r, secondaryThreeColor.g, secondaryThreeColor.b
      );
    }
  }, [secondaryColor, secondaryThreeColor]);
  ...
  useEffect(() => {
    if (materialRef.current) {
      materialRef.current.uniforms.iHoverIntensity.value = hoverIntensity;
    }
  }, [hoverIntensity]);
  ...
  return <div ref={containerRef} className={`ghost-cursor-enhanced ${className ?? ''}`} style={mergedStyle} />;
};

export default GhostCursorEnhanced;
