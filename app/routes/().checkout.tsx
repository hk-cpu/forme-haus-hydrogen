...
export async function loader({context}: LoaderFunctionArgs) {
  ...
  if (!cartResult || (cartResult.totalQuantity ?? 0) === 0) throw redirect('/cart');
  ...
  return json({
    ...
    subtotal: cartResult.cost.subtotalAmount?.amount ?? cartResult.cost.totalAmount.amount,
...
export async function action({request, context}: ActionFunctionArgs) {
  ...
  const tapPayload: Record<string, unknown> = {
    ...
    shipping: {
      address: {line1, line2: line2 || undefined, city, country_code: 'SA', zip: zip || undefined},
    },
    source: {id: 'src_all'},
    post: {url: `${origin}${wbPath}`},
    redirect: {url: `${origin}${cbPath}?merchantTxId=${encodeURIComponent(merchantTxId)}`},
  };

  const secretKey = env.TAP_SECRET_KEY;
  const apiUrl    = env.TAP_API_URL || 'https://api.tap.company/v2';

  if (!secretKey) return json({error: 'Payment gateway not configured.'}, {status: 500});
  ...
  try {
    ...
    const data = await res.json() as {id?: string; transaction?: {url?: string}; errors?: {description: string}[]};

    if (data.errors?.length) {
      return json({error: data.errors[0]?.description ?? 'Payment initiation failed.'}, {status: 400});
    }
...
const labelCls = 'block text-[10px] uppercase tracking-widest text-taupe mb-1.5';

function Field({id, label, ...props}: React.InputHTMLAttributes<HTMLInputElement> & {label: string}) {
  return (
    <div>
      <label htmlFor={id} className={labelCls}>{label}{props.required && <span className="text-bronze ml-0.5">*</span>}</label>
      <input id={id} name={id} className={inputCls} {...props} />
    </div>
  );
}

const KSA_CITIES = ['Riyadh','Jeddah','Mecca','Medina','Dammam','Khobar','Dhahran','Tabuk','Abha','Taif','Yanbu','Buraydah','Najran','Jizan','Khamis Mushait','Hail','Jubail','Al Kharj'];

type NominatimHit = {place_id: number; display_name: string; address?: {road?: string; suburb?: string; city?: string; town?: string; postcode?: string}};

function AddressAutocomplete({onSelect}: {onSelect: (h: NominatimHit) => void}) {
    ...
    <div className="relative">
      ...
      <div className="relative">
        <input
          ...
          autoComplete="off"
        />
        {busy && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 border-2 border-bronze/30 border-t-bronze rounded-full animate-spin" />}
      </div>
      {hits.length > 0 && (
        <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-[#1C1714] border border-taupe/20 rounded-lg shadow-2xl overflow-hidden">
          {hits.map(h => (
            <button key={h.place_id} type="button"
              ...
              onClick={() => { onSelect(h); setQ(h.display_name.split(',').slice(0,2).join(',')); setHits([]); }}
...
export default function CheckoutPage() {
  ...
  const handleGPS = useCallback(async () => {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      try {
        ...
        const d = await r.json() as {address?: {road?: string; suburb?: string; neighbourhood?: string; city?: string; town?: string; postcode?: string}};
        setAddr(prev => ({
          ...prev,
          line1: [d.address?.road, d.address?.suburb ?? d.address?.neighbourhood].filter(Boolean).join(', ') || prev.line1,
  ...
  const handleAddressSelect = useCallback((h: NominatimHit) => {
    setAddr(prev => ({
      ...prev,
      line1: [h.address?.road, h.address?.suburb].filter(Boolean).join(', ') || h.display_name.split(',')[0],
  ...
  const formatMoney = (amount: string, curr: string) =>
    new Intl.NumberFormat('en-SA', {style: 'currency', currency: curr}).format(parseFloat(amount));
    ...
    <div className="min-h-screen bg-background pt-24 pb-16 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10 text-center">
          <h1 className="font-serif text-3xl md:text-4xl text-warm mb-2">Secure Checkout</h1>
          <p className="text-taupe text-sm">Your order stays 100% on Forme Haus</p>
        </div>
            ...
            return (
              <div key={label} className="flex items-center">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full text-[11px] uppercase tracking-widest transition-all ${active ? 'bg-bronze text-white' : done ? 'text-bronze cursor-pointer' : 'text-taupe/40 cursor-default'}`}
                >
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${active ? 'bg-white text-bronze' : done ? 'bg-bronze text-white' : 'bg-taupe/20 text-taupe'}`}>
                    {done ? '✓' : n}
                  </span>
                  {label}
                </button>
                {i < 2 && <div className={`w-8 h-px ${step > n ? 'bg-bronze' : 'bg-taupe/20'}`} />}
              </div>
            );
          })}
        </div>
            ...
            <AnimatePresence mode="wait">
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <div className="flex items-center px-3 bg-surface border border-taupe/20 rounded-lg text-taupe text-sm whitespace-nowrap">🇸🇦 +966</div>
                        <input id="phone" name="phone" type="tel" placeholder="5X XXX XXXX" className={inputCls} value={contact.phone} onChange={e => setContact(c => ({...c, phone: e.target.value}))} />
                      </div>
                    </div>
                  </div>
                  <button
              ...
              {step === 2 && (
                <motion.div key="s2" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{duration: 0.3}}>
                  <h2 className="font-serif text-xl text-warm mb-6">Shipping Address</h2>
                  ...
                  >
                    {gpsLoading ? (
                      <><div className="w-4 h-4 border-2 border-bronze/30 border-t-bronze rounded-full animate-spin" /> Detecting location…</>
                    ) : (
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M1 12h4M19 12h4"/></svg> Use my current location</>
                    )}
                  </button>
                  {gpsError && <p className="text-red-400 text-xs mb-4">{gpsError}</p>}

                  <div className="space-y-4 mb-8">
                    <AddressAutocomplete onSelect={handleAddressSelect} />
                    <Field id="line1" label="Street Address" required placeholder="Building / Street" value={addr.line1} onChange={e => setAddr(a => ({...a, line1: e.target.value}))} />
                    <Field id="line2" label="Apt / Unit (optional)" placeholder="Apartment, Suite, etc." value={addr.line2} onChange={e => setAddr(a => ({...a, line2: e.target.value}))} />
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label htmlFor="city" className={labelCls}>City<span className="text-bronze ml-0.5">*</span></label>
                        <select id="city" name="city" value={addr.city} onChange={e => setAddr(a => ({...a, city: e.target.value}))} className={inputCls + ' cursor-pointer'}>
                          {KSA_CITIES.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </div>
                      <Field id="zip" label="Postal Code" placeholder="12345" value={addr.zip} onChange={e => setAddr(a => ({...a, zip: e.target.value}))} />
                    </div>
                  </div>

                  <div className="flex gap-3">
                    <button type="button" onClick={() => setStep(1)} className="px-6 py-4 border border-taupe/20 text-taupe rounded-xl text-[11px] uppercase tracking-widest hover:bg-taupe/10 transition-colors">← Back</button>
                    <button
                      type="button"
              ...
              {step === 3 && (
                <motion.div key="s3" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{duration: 0.3}}>
                  <h2 className="font-serif text-xl text-warm mb-6">Review & Pay</h2>

                  {/* Summary boxes */}
                  <div className="grid sm:grid-cols-2 gap-4 mb-6">
                    <div className="p-4 bg-surface rounded-xl border border-taupe/10">
                      <p className="text-[10px] uppercase tracking-widest text-taupe mb-2">Contact</p>
                      <p className="text-warm text-sm">{contact.firstName} {contact.lastName}</p>
                      <p className="text-taupe text-xs">{contact.email}</p>
                      <p className="text-taupe text-xs">{contact.phone}</p>
                      <button type="button" onClick={() => setStep(1)} className="text-[10px] text-bronze hover:underline mt-1">Edit</button>
                    </div>
                    <div className="p-4 bg-surface rounded-xl border border-taupe/10">
                      <p className="text-[10px] uppercase tracking-widest text-taupe mb-2">Shipping To</p>
                      <p className="text-warm text-sm">{addr.line1}{addr.line2 ? `, ${addr.line2}` : ''}</p>
                      <p className="text-taupe text-xs">{addr.city}, Saudi Arabia {addr.zip}</p>
                      <button type="button" onClick={() => setStep(2)} className="text-[10px] text-bronze hover:underline mt-1">Edit</button>
                    </div>
                  </div>

                  {actionErr && (
                    <div className="mb-4 p-3 bg-red-400/10 border border-red-400/20 rounded-lg text-red-400 text-xs text-center">{actionErr}</div>
                  )}
                  ...
                  <fetcher.Form method="post">
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={() => setStep(2)} className="px-6 py-4 border border-taupe/20 text-taupe rounded-xl text-[11px] uppercase tracking-widest hover:bg-taupe/10 transition-colors">← Back</button>
                      <button
                        type="submit"
                      ...
                      >
                        {isSubmitting ? (
                          ...
                          <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> Pay {formatMoney(total, currency)}</>
                        )}
                      </button>
                    </div>
                  </fetcher.Form>
                  ...
                  <div className="flex items-center justify-center gap-2 mt-4 flex-wrap">
                    {['mada','Visa','Mastercard','Apple Pay','STC Pay'].map(m => (
                      <span key={m} className="px-2 py-1 text-[9px] font-medium text-taupe bg-surface border border-taupe/10 rounded">{m}</span>
                    ))}
                  </div>
                </motion.div>
              )}
            ...
            <div className="space-y-3 mb-4">
              {lines.map((line: any) => (
                <div key={line.id} className="flex gap-3">
                  {line.image && <img src={line.image} alt={line.title} className="w-14 h-14 object-cover rounded-lg bg-taupe/10 flex-shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <p className="text-warm text-xs truncate">{line.title}</p>
                    {line.variantTitle && line.variantTitle !== 'Default Title' && (
                      <p className="text-taupe text-[10px]">{line.variantTitle}</p>
                    )}
                    <p className="text-taupe text-[10px]">Qty {line.quantity}</p>
                  </div>
                  <p className="text-warm text-xs whitespace-nowrap">{formatMoney(line.price, line.currency)}</p>
                </div>
              ))}
            </div>
            <div className="border-t border-taupe/10 pt-4 space-y-2">
              <div className="flex justify-between text-xs text-taupe"><span>Subtotal</span><span>{formatMoney(subtotal, currency)}</span></div>
              <div className="flex justify-between text-xs text-taupe">
                <span>Shipping</span>
                <span>{parseFloat(subtotal) >= 250 ? <span className="text-bronze">Free</span> : formatMoney('25', currency)}</span>
              </div>
