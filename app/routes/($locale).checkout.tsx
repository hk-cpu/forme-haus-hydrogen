...
export async function action({request, context}: ActionFunctionArgs) {
  ...
  let merchantTxId = `FH-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 6)
    .toUpperCase()}`;
  ...
  if (adminToken && storeDomain) {
    ...
    try {
      ...
      if (draftData.draft_order?.id) {
        ...
        console.warn(
          '[Checkout] Draft order creation returned no ID:',
          draftData,
        );
      }
    } catch (e) {
...
function AddressAutocomplete({
  ...
  return (
    <div className="relative">
      <label htmlFor="address-search-input" className={labelCls}>
        Search Address
      </label>
      <div className="relative">
        <input
      ...
      {error && <p className="mt-1 text-[11px] text-taupe/60">{error}</p>}
    </div>
  );
}
                ...
                >
                  ...
                  <label className="flex items-start gap-3 mb-6 cursor-pointer">
                    ...
                    <span className="text-[11px] text-taupe leading-relaxed">
                      Keep me updated with new arrivals, exclusive offers, and
                      style edits from Formé Haus.
                    </span>
                  </label>
