...
async function test() {
  ...
  console.log("Normal 'phone' search:", JSON.stringify(data1.data?.products?.nodes?.map(n => n.title), null, 2));

  // Targeted
  const formattedSearchTerm = searchInput.split(' ').filter(w => w.trim().length > 0).map(word => `(title:*${word}* OR product_type:*${word}* OR tag:*${word}*)`).join(' AND ');
  ...
  console.log("Targeted search:", JSON.stringify(data2.data?.products?.nodes?.map(n => n.title), null, 2));
}
