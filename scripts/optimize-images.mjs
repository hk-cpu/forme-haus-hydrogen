...
async function optimizeImages() {
    ...
    for (const file of imageFiles) {
        ...
        if (sizeKB > 500) {
            console.log(`🔹 [${file}] - Current size: ${(sizeKB / 1024).toFixed(2)} MB`);
            ...
            if (file.toLowerCase().endsWith('.png')) {
                await pipeline.png({ quality: QUALITY, compressionLevel: 9 }).toFile(`${filePath}.opt`);
            } else {
                await pipeline.jpeg({ quality: QUALITY, progressive: true }).toFile(`${filePath}.opt`);
            }
